...
class ConllSrlReader(ConllReader):
    def __init__(self, index_field_map, pred_start, pred_end=0, pred_key=PREDICATE_KEY):
        ...
        self.predicate_mapping_fn = None
    ...
    def read_instances(self, rows):
        ...
        for key, labels in self.read_predicates(rows).items():
            ...
            if self.predicate_mapping_fn:
                mapped = self.predicate_mapping_fn(key, instance)
                instance[WORD_KEY] = [index == key and mapped or word for index, word in enumerate(instance[WORD_KEY])]
    ...
    @staticmethod
    def predicate_mapping_function(mappings):
        return lambda index, instance: mappings.get("{}.{}".format(instance[PREDICATE_KEY][index], instance[ROLESET_KEY][index]),
                                                    instance[WORD_KEY][index])
