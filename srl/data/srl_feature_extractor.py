...
from readers import Conll2005Reader, Conll2012Reader, ConllPhraseReader, ConllSrlReader, CustomSrlReader
from srl.common.srl_utils import read_mappings, serialize
...
def main(flags):
    ...
    mapping_fn = None if not flags.mappings else ConllSrlReader.predicate_mapping_function(read_mappings(flags.mappings))

    if flags.dataset == 'conll2012':
        reader = Conll2012Reader()
        reader.predicate_mapping_fn = mapping_fn
        raw_instances = reader.read_files(flags.input, flags.ext)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mappings', required=False, type=str, help='Predicate mappings file.')
